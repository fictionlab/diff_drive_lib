...
void MecanumController::setSpeed(const float linear_x, const float linear_y,
                                 const float angular) {
  ...
  const float wheel_FL_vel =
      (diff_xy - angular_multiplied) / params_.robot_wheel_radius;
  const float wheel_FR_vel =
      (sum_xy + angular_multiplied) / params_.robot_wheel_radius;
  const float wheel_RL_vel =
      (sum_xy - angular_multiplied) / params_.robot_wheel_radius;
  const float wheel_RR_vel =
      (diff_xy + angular_multiplied) / params_.robot_wheel_radius;
...
void MecanumController::update(uint32_t dt_ms) {
  ...
  const float x_move = velocity_lin_x * std::cos(odom_.pose_yaw) -
                       velocity_lin_y * std::sin(odom_.pose_yaw);
  const float y_move = velocity_lin_x * std::sin(odom_.pose_yaw) +
                       velocity_lin_y * std::cos(odom_.pose_yaw);
